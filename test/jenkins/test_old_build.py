# Testing for the Jenkins old_build.py script
#
# RUN: export TESTING=1
# RUN: export JOB_NAME="FOO"
# RUN: export BUILD_NUMBER=321
# RUN: export BRANCH=master
# Tell old_build.py to just print commands instead of running.
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang all > %t.log
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang all --assertions > %t-assert.log
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang all --lto > %t-lto.log
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang build --lto
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang build

# Just a test
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang test

# CMake

# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake all --debug > %t-cmake.log
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake build
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake test
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake testlong

# Derive Functions

# RUN: cd %t.SANDBOX; python %{src_root}/zorg/jenkins/old_build.py derive > %t-derive.log
# RUN: FileCheck --check-prefix CHECK-DERIVE < %t-derive.log %s
# CHECK-DERIVE: @@@ Derive Source @@@
# CHECK-DERIVE: cd
# CHCEK-DERIVE: Output/test_old_build.py.tmp.SANDBOX/llvm
...
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/llvm'
# CHECK-DERIVE: 'rsync' '-auvh' '--delete' '--exclude=.svn/' '--exclude=/tools/clang/tools/extra'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/clang.src/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/llvm/tools/clang'
# CHECK-DERIVE: 'rsync' '-auvh' '--delete' '--exclude=.svn/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/libcxx.src/'
# CHECK-DERIVE:test_old_build.py.tmp.SANDBOX/llvm/projects/libcxx'
# CHECK-DERIVE: 'rsync' '-auvh' '--delete' '--exclude=.svn/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/clang-tools-extra.src/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/llvm/tools/clang/tools/extra'
# CHECK-DERIVE: 'rsync' '-auvh' '--delete' '--exclude=.svn/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/compiler-rt.src/'
# CHECK-DERIVE: test_old_build.py.tmp.SANDBOX/llvm/projects/compiler-rt'
# CHECK-DERIVE: @@@@@@



# RUN: cd %t.SANDBOX; python %{src_root}/zorg/jenkins/old_build.py derive-lldb
# RUN: cd %t.SANDBOX; python %{src_root}/zorg/jenkins/old_build.py derive-llvm+clang
# RUN: cd %t.SANDBOX; python %{src_root}/zorg/jenkins/old_build.py derive-llvm

# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake all --lto | FileCheck --check-prefix CHECK-CMAKELTO %s
...
# RUN: env MAX_PARALLEL_LINKS=2 python %{src_root}/zorg/jenkins/old_build.py cmake all --lto | FileCheck --check-prefix CHECK-CMAKE-PAR-LTO %s
...
# RUN: env MAX_PARALLEL_TESTS=2 python %{src_root}/zorg/jenkins/old_build.py cmake all | FileCheck --check-prefix CHECK-CMAKE-2-TESTS %s
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake all --cmake-type=RelWithDebugInfo | FileCheck --check-prefix CHECK-CMAKE-UPLOADS %s
...
# RUN: python %{src_root}/zorg/jenkins/old_build.py clang all --lto --cmake-flag="-DFOO" | FileCheck --check-prefix CHECK-CMAKEFLAGS %s
# CHECK-CMAKEFLAGS: '-DFOO'

# Make sure you can pass new build targetss.
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake  build --cmake-build-target foo --cmake-build-target bar | FileCheck --check-prefix CHECK-BTARGETS %s
# CHECK-BTARGETS: '/usr/local/bin/ninja' '-v' 'foo' 'bar'

# Make sure you can pass new test targets.
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake test --cmake-test-target foo --cmake-test-target bar | FileCheck --check-prefix CHECK-TTARGETS %s
# CHECK-TTARGETS: '/usr/local/bin/ninja' '-v' '-k' '0' 'foo' 'bar'

# Test long should always do check-all, since that is what many bots expect.
# RUN: python %{src_root}/zorg/jenkins/old_build.py cmake testlong  | FileCheck --check-prefix CHECK-TTARGETS2 %s
